...
**Список** отбирает сотрудников по офису **текущего основного назначения** —
определение «текущего» вынесено в `current_primary_assignment_predicate` и одно
на весь модуль. **Карточка** доступна шире: по любому назначению в истории,
попавшему в область видимости.

Расхождение намеренное. Сузить карточку до текущего назначения нельзя:
увольнение закрывает все открытые периоды, и тогда HR своего же региона потерял
бы доступ к карточкам собственных уволенных сотрудников. Переведённый в другой
регион человек по той же причине остаётся читаемым для прежнего региона, но в
списке текущего состава уже не появляется. Закреплено тестами
`test_transferred_out_employee_stays_readable_for_former_region` и
`test_terminated_employee_stays_visible_to_regional_hr`.
