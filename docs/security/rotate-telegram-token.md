...
- **все сессии CRM обрываются** — каждый HR входит заново (хэш сессии
  подписан ключом);
- **все Bearer Mini App недействительны** (`humotech/telegram/tokens.py`
  подписывает их `django.core.signing` этим ключом) — сотрудникам нужно
  переоткрыть Mini App, вход пройдёт автоматически через `initData`;
...
1. `python3 -c "import secrets; print(secrets.token_urlsafe(64))"` → в
   `apps/backend-api/.env`, `DJANGO_SECRET_KEY=` (редактором, как выше).
2. `$DC up -d --force-recreate $BACKEND_SERVICES`.
3. Проверка: CRM открывается страницей входа, вход работает; Mini App после
   переоткрытия пускает; `docker exec humotech_backend python manage.py check --deploy`
   не жалуется на `SECRET_KEY`.

Плавная смена без разлогинивания возможна через Django
`SECRET_KEY_FALLBACKS` (старый ключ в списке на срок жизни сессии), но
`config/settings/production.py` сейчас эту настройку не читает. При
**утечке** ключа плавная смена не нужна: старый ключ должен перестать
действовать сразу.
