...
Кнопка личного кабинета — `web_app`, и она первая: это главный вход.
Остальные — обычные текстовые: их видно в истории чата, и по ним можно
вернуться к прошлому ответу, не нажимая заново.
"""
...
def employee_menu(mini_app_url: str | None) -> ReplyKeyboardMarkup:
    ...
    if mini_app_url:
        rows.append(
            [KeyboardButton(text=BTN_CABINET, web_app=WebAppInfo(url=mini_app_url))]
        )
    rows.extend(
        [
