...
Ведёт на быструю отметку, а не в кабинет целиком. Причина простая:
кабинет открывают изредка, а отмечаются дважды в день, каждый рабочий
день. Место у поля ввода одно на бота — его занимает то, что делают
чаще. Кабинет никуда не делся: `/cabinet` и кнопка внизу.

Почему именно кнопка меню, а не кнопка нижней клавиатуры. Telegram
передаёт подписанную строку запуска только приложениям, открытым из
inline-кнопки, кнопки меню или прямой ссылки. Открытое из нижней
клавиатуры не получает ни подписи, ни имени — и отметиться не смогло бы
вовсе: сервер определяет сотрудника по подписи, и брать его больше
неоткуда (см. `keyboards/employee.py`).

Inline-кнопка для постоянного входа не годится по другой причине: она
...
BUTTON_TEXT = "Отметиться"

#: Путь быстрой отметки внутри Mini App. Домен сюда не пишется: он
#: приходит из MINI_APP_URL и меняется вместе с туннелем.
SCAN_PATH = "/scan"
