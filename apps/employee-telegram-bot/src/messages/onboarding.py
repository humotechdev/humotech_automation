...
INFO_DONE = (
    ...
    "Вы прочитали 10 из 10 информационных разделов.\n\n"
    "Далее необходимо ознакомиться и подтвердить\n"
    "3 обязательных документа.\n\n"
    "Без их подтверждения рабочие функции бота пока недоступны."
)
...
def info_done(state: dict) -> str:
    ...
    return (
        ...
        f"Далее необходимо ознакомиться и подтвердить\n"
...
DECLINED = (
    ...
    "Доступ к рабочим функциям пока ограничен.\n\n"
    "HR получил уведомление и свяжется с вами."
)
...
FINISHED = (
    ...
    "Теперь вам доступны рабочие функции бота."
)

#: Что показать тому, кто жмёт рабочую кнопку до завершения.
BLOCKED = (
    "Рабочие функции откроются после первичного ознакомления.\n\n"
    "Нажмите «Продолжить ознакомление» — это займёт несколько минут."
)
BLOCKED_DECLINED = (
    "Вы не подтвердили обязательный документ, поэтому рабочие функции "
    "закрыты.\n\nОтдел кадров уже знает и свяжется с вами. "
    "Подтвердить документ можно в разделе «Правила и документы»."
)
...
MENU_INSTALLED = (
    "Ниже — меню ознакомления.\n\n"
    "Прогресс сохраняется: можно закрыть чат и продолжить в любой момент."
)
...
HR_CONTACTS = (
    "<b>Контакты отдела кадров</b>\n\n"
    "Напишите в HR прямо здесь: нажмите «✍️ Написать в HR» в меню — "
    "сообщение попадёт в очередь обращений, и кадровик ответит в этот чат.\n\n"
    "По вопросам логина, пароля и доступа к рабочим системам обращайтесь "
