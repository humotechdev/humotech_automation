...
from src.modules.ai_assistant.services.cache import (
    CacheKeyParts,
...
def test_ttl_expiry(parts, monkeypatch):
    ...
    import src.modules.ai_assistant.services.cache as cache_module
...
def test_redis_backend_fails_loudly(settings):
    """Молчаливый откат на память в проде хуже явной ошибки."""
    settings = settings.model_copy(update={"ai_cache_backend": "redis"})
