...
from src.modules.ai_assistant.models import AnswerFeedback, LlmQueryLog
...
class FeedbackUseCase:
    ...
    def execute(self, request: FeedbackRequest) -> FeedbackResponse:
        ...
        log = self.session.get(LlmQueryLog, request.query_log_id)
        if log is None:
        ...
        existing = self.session.scalar(
            select(AnswerFeedback).where(
                AnswerFeedback.query_log_id == request.query_log_id,
                AnswerFeedback.employee_id == request.employee_id,
            )
        )
...
class HealthUseCase:
    ...
    def __init__(
        self, session: Session, settings: AiSettings | None = None
    ) -> None:
        self.session = session
        self.settings = settings or ai_settings
