...
from src.modules.ai_assistant.services.cache import CacheService, build_cache
...
from src.modules.ai_assistant.use_cases.bot import (
    AnswerUseCase,
    FeedbackUseCase,
    HealthUseCase,
)
from src.modules.ai_assistant.use_cases.crm import KnowledgeAdminUseCases
...
def build_personal_data_service(
    session: Session, settings: AiSettings | None = None
) -> PersonalDataQueryService:
    ...
    return SqlPersonalDataQueryService(session)
...
def build_container(
    ...
    *,
    ...
    """Собирает модуль под одну сессию.
    ...
    if llm is not None and embeddings is not None:
        answer_service = AnswerService(
            ...
            cache=cache,
            personal_data_service=build_personal_data_service(session, settings),
            settings=settings,
    ...
    return AiContainer(
        ...
        feedback=FeedbackUseCase(session),
        health=HealthUseCase(session, settings),
