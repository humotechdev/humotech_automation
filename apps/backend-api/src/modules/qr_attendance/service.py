...
from src.modules.employees.models import EmployeeAssignment, EmployeeOfficeAccess
    ...
    *,
    ...
    assignment = session.scalar(
        select(EmployeeAssignment.id).where(
            ...
            EmployeeAssignment.office_id == office_id,
    ...
    access = session.scalar(
        select(EmployeeOfficeAccess.id).where(
            EmployeeOfficeAccess.employee_id == employee_id,
            EmployeeOfficeAccess.office_id == office_id,
            EmployeeOfficeAccess.valid_from <= at,
            (EmployeeOfficeAccess.valid_to.is_(None))
            | (EmployeeOfficeAccess.valid_to >= at),
        )
    )
...
def _nonce_already_used(
    session: Session, *, employee_id: uuid.UUID, nonce_hash: str, event_type: str
) -> bool:
    return session.scalar(
        select(AttendanceEvent.id).where(
            AttendanceEvent.employee_id == employee_id,
            AttendanceEvent.qr_nonce_hash == nonce_hash,
            AttendanceEvent.event_type == event_type,
            AttendanceEvent.verification_status == "ACCEPTED",
        )
    ) is not None
...
def register_scan(
    ...
    *,
    ...
    current_session = open_session_for(session, employee_id=employee_id)
    ...
    if event_type == "ENTRY":
        work_session = AttendanceSession(
            organization_id=qr_point.organization_id,
            employee_id=employee_id,
            office_id=office_id,
...
def _reject_reason(
    ...
    *,
    ...
    if not employee_may_use_office(
        session, employee_id=employee_id, office_id=qr_point.office_id, at=now
    ):
