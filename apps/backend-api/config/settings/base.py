...
HUMOTECH_APPS = [
    ...
    "humotech.reports",
    "humotech.surveys",
    # Первичное ознакомление: десять информационных карточек и три
    # обязательных документа. Стоит после `telegram` намеренно — ссылку
    # на ознакомление выдаёт тот же механизм приглашений.
    "humotech.onboarding",
]
...
TELEGRAM = {
    ...
    # Ссылка на первичное ознакомление живёт дольше — неделю. Её
    # отправляют до выхода человека на работу, иногда в пятницу, и
    # суточный срок обычной привязки не пережил бы выходные.
    "ONBOARDING_INVITATION_TTL_SECONDS": int(
        env("TELEGRAM_ONBOARDING_INVITATION_TTL_SECONDS", "604800")
    ),
...
QR = {
    ...
    # Потолок погрешности телефона. Погрешность прибавляется к радиусу
    # геозоны — иначе человека у самой двери отвергал бы обычный сбой
    # GPS, — и без потолка «accuracy: 50000» проглотила бы любой радиус,
    # превратив проверку в её видимость.
    #
    # Сто метров: столько показывает телефон в здании по вышкам и Wi-Fi,
    # и это ещё осмысленный сигнал. Больше — уже не «около офиса»,
    # а «где-то в городе».
    "MAX_LOCATION_ACCURACY_M": int(env("QR_MAX_LOCATION_ACCURACY_M", "100")),
}
...
#
# Доступа ознакомление не закрывает, поэтому довести его до конца может
# только напоминание. Все три величины здесь — про то, чтобы оно не
# стало назойливым.
ONBOARDING = {
    # Пауза между напоминаниями одному человеку. Трое суток: за меньшее
    # время сообщение читается как «нас не услышали», за большее —
    # ознакомление растягивается на месяц.
    "REMINDER_INTERVAL_HOURS": int(env("ONBOARDING_REMINDER_INTERVAL_HOURS", "72")),
    # Сколько ждать после приглашения, прежде чем торопить. Сутки:
    # человек мог получить ссылку в конце дня.
    "REMINDER_GRACE_HOURS": int(env("ONBOARDING_REMINDER_GRACE_HOURS", "24")),
    # Рабочие часы в поясе организации. «Дочитайте правила» в три часа
    # ночи — это не напоминание, а причина отключить бота.
    "REMINDER_FROM_HOUR": int(env("ONBOARDING_REMINDER_FROM_HOUR", "10")),
    "REMINDER_TO_HOUR": int(env("ONBOARDING_REMINDER_TO_HOUR", "19")),
}
