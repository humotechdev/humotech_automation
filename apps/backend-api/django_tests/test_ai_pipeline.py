"""Конвейер ответа: на дублёрах провайдеров, но на живой базе.

Раньше журнал и эскалация проверялись через поддельную сессию. Теперь
их пишет ORM, поэтому тесты работают с настоящими строками — заодно это
проверяет, что записи вообще проходят ограничения схемы.
...
from humotech.ai_assistant.models import LlmQueryLog, UnansweredQuestion
...
@pytest.fixture()
def live_scope(organization, employee):
    """Область настоящего сотрудника: журнал ссылается на него внешним ключом."""
...
@pytest.fixture()
def scope(live_scope):
    return live_scope


@pytest.fixture()
def build(monkeypatch, live_scope, ai_settings):
    """Собирает AnswerService с подменённым поиском и дублёрами провайдеров."""

    def _build(*, llm=None, retrieval=None, cache=None, personal=None, cfg=None):
        cfg = cfg or ai_settings
        monkeypatch.setattr(
            answer_module, "resolve_employee_scope", lambda **kw: live_scope
        )
        monkeypatch.setattr(answer_module, "current_revision", lambda org: 5)
...
def test_no_sources_escalates_to_hr(build, scope):
    ...
    assert UnansweredQuestion.objects.count() == 1


def test_conflicting_rules_escalate_without_asking_the_model(build, scope):
    llm = FakeLLMProvider()
    ...
    assert list(UnansweredQuestion.objects.all())
...
def test_fallback_is_called_at_most_once(build, scope, ai_settings):
    ...
    assert llm.calls[0]["model"] == ai_settings.openai_chat_model
    assert llm.calls[1]["model"] == ai_settings.openai_fallback_model
    assert response.fallback_used is True
    assert response.status is AnswerStatus.ESCALATED


def test_fallback_disabled_means_single_call(build, scope, ai_settings):
    llm = FakeLLMProvider(
        answer=json.dumps({"answer": "", "answered": False, "used_fragments": []})
    )
...
def test_unavailable_llm_returns_safe_answer(build, scope):
    result = RetrievalResult(chunks=(make_chunk(),), top_score=0.88)
...
def test_provider_error_is_recorded_with_code_not_message(build, scope):
...
def test_secrets_never_reach_the_query_log(build, scope):
    llm = FakeLLMProvider(
        answer=json.dumps({"answer": "ок", "answered": True, "used_fragments": []})
    )
    result = RetrievalResult(chunks=(make_chunk(),), top_score=0.88)
...
def test_log_records_prompt_version_and_tokens(build, scope):
    llm = FakeLLMProvider(
        answer=json.dumps({"answer": "ок", "answered": True, "used_fragments": []})
    )
    result = RetrievalResult(chunks=(make_chunk(),), top_score=0.88)
