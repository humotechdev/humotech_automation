"""Внешние ключи сообщений о дне — в том же виде, что и остальные.

Django задаёт `on_delete` на стороне Python, а в DDL пишет
`DEFERRABLE INITIALLY DEFERRED` без всякого `ON DELETE`. Здесь так
...
Оба ключа RESTRICT. Сказанное человеком про свой день объясняет пустую
строку в табеле, и исчезнуть вместе с чем-то удалённым мимо ORM оно не
должно: без него у дня остаётся необъяснённый прогул.
"""
...
FOREIGN_KEYS = [
    ('attendance_day_notices', 'fk_attendance_day_notices_organization_id',
     'organization_id', 'organizations', 'id', 'RESTRICT'),
    ('attendance_day_notices', 'fk_attendance_day_notices_employee_id',
