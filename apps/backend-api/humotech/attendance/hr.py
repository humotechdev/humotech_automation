...
class AttendanceHrService(BaseService):
    ...
    def presence(
        ...
        *,
        ...
        Постраничного вывода здесь нет намеренно: ответ — это состав смены
        одного офиса на один день, и он нужен целиком. Разрезать его на
        страницы значило бы считать итоги дашборда по первым пятидесяти
        строкам. Ограничение вместо страниц — обязательная область: без
        офиса или региона запрос идёт по видимой области целиком, и её
        размер задаёт не клиент, а права.
        """
