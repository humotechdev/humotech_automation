"""Внешний ключ истории попыток — в том же виде, что и остальные ключи схемы.
...
RESTRICT, а не CASCADE: попытка отправки — часть истории уведомления.
Удаление строки, за которой стоят состоявшиеся отправки, должно
отклоняться, а не молча уносить с собой объяснение, почему сообщение
не дошло.
"""
...
FOREIGN_KEYS = [
    ('notification_attempts', 'fk_notification_attempts_notification_id',
     'notification_id', 'notifications', 'id', 'RESTRICT'),
]
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('notifications', '0003_attempt_history'),
