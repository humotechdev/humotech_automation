...
Чего команда НЕ находит — и почему:

  * продление больничного (`absence.request.extend`). Уведомление уходит
    по ДОЧЕРНЕЙ заявке, а в журнале записан идентификатор родительской;
  * отклонение приглашения (`telegram.invitation.reject`). Там уведомление
    ставится в очередь ТОЛЬКО если привязка уже существовала, а по одной
    записи журнала это не видно.

В обоих случаях запись журнала есть, а вывода «строка была» из неё не
следует. Объявлять такие строки пропавшими нельзя: это выдумало бы
уведомления, которых могло не быть вовсе. Искать их надо по самим
заявкам и привязкам, вручную.
