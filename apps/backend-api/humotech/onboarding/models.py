...
class EmployeeOnboarding(
    ...
    ознакомление». Это существенно: гейт закрывает бота ТОЛЬКО тем, у
    кого строка есть. Сотрудники, работавшие до появления раздела,
    остаются работать — включить их в программу можно, но это отдельное
    решение кадровика, а не побочный эффект выката.

    `status` — вычисляемая колонка, см. модуль `progress`. Она нужна
    спискам («покажи всех, кто не начал») и фильтрам, но решение о
    допуске принимается пересчётом.
