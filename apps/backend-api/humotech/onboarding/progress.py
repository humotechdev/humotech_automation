"""Сколько пройдено. Единственное место, где это считают.
...
обновляется, но он — витрина для списков и фильтров. Стоит кому-нибудь
начать принимать решения по колонке, и выпуск новой редакции
обязательного документа перестанет кого-либо касаться: у всех, кто
прошёл ознакомление год назад, там навсегда останется COMPLETED.
...
**Доступа этот модуль не раздаёт и не отбирает.** Незавершённое
ознакомление — повод напомнить человеку и показать его кадровику, а не
повод закрыть отметку присутствия: новичок отмечается на входе в
первый же день, ещё не открыв ни одной карточки. Проверка «пока не
дочитал — нельзя» здесь стояла и была снята намеренно.

Кто считается участником. Только те, у кого есть строка
`employee_onboarding`, то есть кого в программу позвали. Сотрудники,
работавшие до появления раздела, в ней не числятся; включить их —
отдельное решение кадровика, а не побочный эффект выката.
"""
...
class Progress:
    ...
    «сколько пройдено» для списка HR, и «что показать дальше» для бота,
    и «напоминать ли». Три отдельных запроса на эти три вопроса
    ...
    def completed(self) -> bool:
        """Закончено ли ознакомление на сегодняшний день.

        Доступа не касается: бот открыт и тому, у кого здесь `False`.
        От этого значения зависит только то, напоминать ли человеку и
        показывать ли его кадровику среди требующих внимания.
        """
    ...
    def status(self) -> str:
        ...
        if self.completed:
            return "COMPLETED"
        # Прошёл всё однажды, а потом вышла новая редакция документа.
        # Это не «застрял на середине»: человек своё сделал, и просят у
        # него теперь другое — подтвердить изменившийся текст.
        if self.onboarding.completed_at is not None:
    ...
    def stage(self) -> str:
        ...
        Четыре значения вместо семи статусов: боту нужно выбрать экран,
        а не назвать состояние. `BLOCKED` от `POLICIES` отличается
        тем, что кнопки согласия в нём не ведут вперёд сами по себе —
        там уже решает кадровик. Доступа к боту ни одно из значений не
        ограничивает.
        """
        if self.completed:
            return "DONE"
...
def of_employee(employee_id: uuid.UUID) -> Progress | None:
    """Состояние одного сотрудника. `None` — человек вне программы.

    Отдельная функция, а не «позови compute», чтобы вызывающему не
    приходилось помнить про случай «строки нет»: её отсутствие значит
    не «ничего не прошёл», а «в программу не звали».
    """
    onboarding = (
...
def is_unfinished(employee_id: uuid.UUID) -> bool:
    """Осталось ли человеку что-то сделать по ознакомлению.

    Доступа это НЕ ограничивает — ни к боту, ни к кабинету. Признак
    нужен напоминанию и спискам кадровика: «не закончил» и «нельзя
    работать» — разные утверждения, и склеивать их оказалось дорого.

    Имя намеренно про незаконченность, а не про «требует внимания»:
    второе — вопрос интерфейса, и отвечает на него список состояний,
    а не эта функция.
    """
    progress = of_employee(employee_id)
