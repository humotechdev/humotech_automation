...
Сюда же переводится `IntegrityError`. Наружу сырое исключение выходить
не должно: в нём текст PostgreSQL, имя ограничения и фрагмент SQL — для
интерфейса это бесполезно, а иногда и небезопасно.

Имя нарушенного ограничения Django не показывает сам: он оборачивает ошибку
psycopg в свою, поэтому диагностику приходится доставать из `__cause__`.
"""
...
class AiDisabled(DomainError):
    """Ассистент выключен рубильником `AI_ASSISTANT_ENABLED`.

    Отдельный код, а не общий отказ: выключенный ассистент — это
    состояние стенда, а не ошибка кадровика, и интерфейс должен сказать
    «ассистент выключен», а не «что-то пошло не так».

    503, а не 400: запрос правильный, просто выполнить его сейчас нечем.
    """
...
class ProviderNotConfigured(DomainError):
    """Рубильник включён, но провайдер настроен не до конца.

    Отличается от `AiDisabled` намеренно. «Выключено» чинит тот, кто
    принимает решение; «не настроено» — тот, у кого есть ключ. Один код
    на оба случая отправлял бы обоих не туда.

    Молчаливой подмены модели здесь не бывает: она опаснее явного отказа.
    """
...
def constraint_name_of(exc: Exception) -> str | None:
    """Имя нарушенного ограничения из диагностики psycopg.

    Django оборачивает исключение драйвера в своё, поэтому исходное лежит
    в `__cause__`. Цепочка обходится целиком: обёртка может оказаться не одна.
    """
