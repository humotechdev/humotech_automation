...
from humotech.ai_assistant.providers.base import EmbeddingProvider
from humotech.ai_assistant.services.chunking import split_text
...
class KnowledgeIngestionService:
    ...
    def index_source(self, source_id: uuid.UUID) -> IngestionResult:
        ...
        for chunk_hash, embedding in KnowledgeChunk.objects.filter(
            organization_id=source.organization_id,
            content_hash__in=[c.content_hash for c in chunks],
        ...
        input_tokens = 0
        if to_embed:
            result = self.embeddings.embed([c.text for c in to_embed], model=model)
        ...
        KnowledgeChunk.objects.filter(source_id=source.id).delete()
