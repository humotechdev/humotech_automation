...
/**
 * Область назначения в форме: только то, что действительно известно.
 *
 * Не `api.Region`: когда справочник регионов недоступен, а офисы
 * читаются, id и название региона берутся из строки офиса — а кода и
 * состояния там нет, и подставлять туда пустые значения значило бы
 * выдумать поля, которых никто не отдавал.
 */
export type ScopeRegion = { id: string; name: string };
